	...
	void insert_dff_cell() {
		...
		switch (insert_dff_type) {
			case INSERT_ADFF:
				...
				new_cell = module->addAdff(NEW_ID, wire_clk_i, wire_rst_ni, wire_d_i, wire_interm, 0);
				break;
			case INSERT_ADFFE:
				// rst_val_sigspec = module->parameter_default_values.find(ID(ARST_VALUE))->second.as_int(), module->parameter_default_values.find(ID(WIDTH))->second.as_int();
				new_cell = module->addAdffe(NEW_ID, wire_clk_i, wire_en_i, wire_rst_ni, wire_d_i, wire_interm, 0); // RTLIL::Const(0xabc, 291));
				// new_cell->setParam(ID::WIDTH, 291);
				break;
			case INSERT_DFFE:
			...
			case INSERT_SDFF:
				...
				new_cell = module->addSdff(NEW_ID, wire_clk_i, wire_rst_ni, wire_d_i, wire_interm, 0);
				break;
			case INSERT_SDFFCE:
				// rst_val_sigspec = module->parameter_default_values.find(ID(SRST_VALUE))->second.as_int(), module->parameter_default_values.find(ID(WIDTH))->second.as_int();
				new_cell = module->addSdffce(NEW_ID, wire_clk_i, wire_en_i, wire_rst_ni, wire_d_i, wire_interm, 0);
				break;
			case INSERT_SDFFE:
				// rst_val_sigspec = module->parameter_default_values.find(ID(SRST_VALUE))->second.as_int(), module->parameter_default_values.find(ID(WIDTH))->second.as_int();
				new_cell = module->addSdffe(NEW_ID, wire_clk_i, wire_en_i, wire_rst_ni, wire_d_i, wire_interm, 0);
				break;
			case INSERT_FF:
				// rst_val_sigspec = module->parameter_default_values.find(ID(SRST_VALUE))->second.as_int(), module->parameter_default_values.find(ID(WIDTH))->second.as_int();
				new_cell = module->addFf(NEW_ID, wire_d_i, wire_interm);
				break;
			default:
